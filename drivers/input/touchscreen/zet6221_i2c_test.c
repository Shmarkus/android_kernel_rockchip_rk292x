...
#define TS_RST_GPIO		RK30_PIN4_PD0//S3C64XX_GPN(10)
...
#if 1
	struct zet6221_tsdrv *ts = (struct zet6221_tsdrv*)dev_id;
...
static void zet6221_ts_work(struct work_struct *_work)
{
	...
	struct zet6221_tsdrv *ts =  container_of(to_delayed_work(_work), struct zet6221_tsdrv, ts_work);
...
static int __devinit zet6221_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//INIT_WORK(&zet6221_ts->work1, zet6221_ts_work);
		...
		zet6221_ts->ts_wq= create_rt_workqueue("rk_ts_wq");		//create a work queue and worker thread
	#else
		zet6221_ts->ts_wq= create_workqueue("rk_ts_wq"); 
	#endif
	if (!zet6221_ts->ts_wq){
		printk(KERN_ALERT "creat zet6221 touch screen workqueue failed\n");
	    return -ENOMEM;
	}
	
	INIT_DELAYED_WORK(&zet6221_ts->ts_work, zet6221_ts_work);
...

{
	...
	result = request_irq(zet6221_ts->irq, zet6221_ts_interrupt,/*IRQF_TRIGGER_FALLING*/IRQF_TRIGGER_LOW, ZET_TS_ID_NAME, zet6221_ts);
	if (result)
		goto request_irq_fail;
