...
extern void Set_ChargeChange_value(int);
//Touch Screen
	...
	if(ret == 0x3C)
	{
		...
		for(i=0;i<FingerNum;i++){
			...
			{		
				...
				input_report_abs(ts->input, ABS_MT_PRESSURE, P_MAX);
...
EXPORT_SYMBOL_GPL(zet6221_ts_charger_mode_disable); ///add by jack
...
static void ts_early_suspend(struct early_suspend *handler)
{
	...
	G_Suspned_Status = 1;  ///> add by jack
	ret=zet6221_i2c_write_tsdata(this_client, ts_sleep_cmd, 1);
    //gpio_direction_output(TS_RST_GPIO, 0);//cckaa
	printk("\n\n Zet6221 %s\n",__FUNCTION__);//cckaa
        return;
...
static void ts_late_resume(struct early_suspend *handler)
{
		...
		gpio_direction_output(TS_RST_GPIO, 1);		
		msleep(20);
		G_Suspned_Status = 0;   //add by jack
#else
		//Wake Up Mode
...

{
	...
	result = gpio_request(zet6221_ts->gpio, NULL); 
	if (result)
	{
		printk("request zet6221_ts INT fail.\n");
...
//JY
	...
	//unregister_early_suspend(&zet6221_ts->early_suspend);//add for test then del
