...
#define TS_RST_GPIO		RK2928_PIN3_PC3//S3C64XX_GPN(10)

#define TPINFO	1
#define  CONFIG_1024x600
#if defined  CONFIG_1024x600
#define X_MAX    1024
#define Y_MAX    600
#elif defined CONFIG_1024x768
#define X_MAX    1024
#define Y_MAX    768
#elif defined CONFIG_1280x800
#define X_MAX    1280
#define Y_MAX    800
#elif defined CONFIG_800x480
#define X_MAX    800
#define Y_MAX    480
#else
#define X_MAX    1024
...
unsigned char G_Suspned_Status = 0;
...
void ctp_reset(void)
{
	...
	//reset mcu
	int ret;
	ret = gpio_request(TS_RST_GPIO, "zet6223 reset pin");
...
static void zet6221_ts_work(struct work_struct *_work)
{
	...
	//			input_report_key(ts->input, BTN_TOUCH , 1); 
				input_report_abs(ts->input, ABS_MT_TRACKING_ID, i);
	    			input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR, 5);
	    			input_report_abs(ts->input, ABS_MT_POSITION_X, px);
	    			input_report_abs(ts->input, ABS_MT_POSITION_Y, py);				   
				input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR, 8);
#ifndef MT_TYPE_B	    		
	//			input_mt_sync(ts->input);
#endif
			}else
			{
				...
				input_mt_sync(ts->input);*/
			//Add by leo
			//	printk("========[leo]===pressure=======touch up (%d)\n",i);
			//end by leo
...
EXPORT_SYMBOL(zet6221_ts_charger_mode);
...
EXPORT_SYMBOL(zet6221_ts_charger_mode_disable);
...
static void ts_late_resume(struct early_suspend *handler)
{
	G_Suspned_Status = 0;
#if 1
        gpio_direction_output(TS_RST_GPIO, 1);//cckaa
	 msleep(1);//cckaa
        gpio_direction_output(TS_RST_GPIO, 0);
		msleep(10);
		gpio_direction_output(TS_RST_GPIO, 1);
...
#endif
      printk("zet6221_resume-------\n");
return;

}
//extern void zet_fw_init(void);
static int __devinit zet6221_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int result;
...
//	ctp_reset(5);
	...
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, P_MAX, 0, 0);	
...
*/
	...
	zet6221_ts->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN - 1,//EARLY_SUSPEND_LEVEL_DISABLE_FB + 2,
	register_early_suspend(&zet6221_ts->early_suspend);
	//disable_irq(zet6221_ts->irq);

//printk ("=============== zet6221_ts_init_ok_111===\n");
probe_finished = 1;
	return 0;
...
gpio_rst_request_fail:	
	kfree(zet6221_ts);
fail_alloc_mem_ts:	
	return result;
}
//extern void zet_fw_exit(void);
static int __devexit zet6221_ts_remove(struct i2c_client *dev)
{
